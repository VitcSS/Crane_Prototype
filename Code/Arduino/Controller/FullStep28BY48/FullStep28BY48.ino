...
void Motor_AHO()                    // Movimento no sentido anti-horário 
{
  for(int i = 0; i < 512; i++)      // incrementa o contador i de 0 a 511 - uma volta
    ...
    for(int j = 0; j < 4; j++)      // incrementa o contador j de 0 a 3 
    {
      ...
      if (Serial.available()) {
        Serial.println("HORARIO: Chegou coisa aqui, ja respondi entao");
...
void Motor_HOR()                    // Movimento no sentido horário 
{
  for(int i = 0; i < 512; i++)      // incrementa o contador i de 0 a 511 - uma volta
    ...
    for(int j = 0; j < 4; j++)      // incrementa o contador j de 0 a 3 
    {
      ...
      delay (atraso_fase);          // Atraso de tempo entre as fases em milisegundos
      if (Serial.available()) {
        Serial.println("ANTI-HORARIO: Chegou coisa aqui, ja respondi entao");
        String data = Serial.readString();
      }
    }
}
...
{
  if (Serial.available()) {
    Serial.println("MAIN: Chegou coisa aqui, ja respondi entao");
    String data = Serial.readString();
  }
  Serial.println("> Chama funcao sentido horario");
  Motor_HOR();           // Gira motor no sentido Horário 
  Serial.println("> Passou da funcao sentido horario");
  delay (intervalo);     // Atraso em milisegundos 
  Serial.println("> Chama funcao sentido anti horario");
  Motor_AHO();           // Gira motor no sentido Anti-Horário 
  Serial.println("> Passou da funcao sentido anti horario");
  delay (intervalo);     // Atraso em milisegundos 
}
